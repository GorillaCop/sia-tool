...
def show_results_page():
    ...
    # Force scroll-to-top after navigation events
    ...
    completed_date = st.session_state.get('assessment_completed_date') or completed_date
    st.markdown(f"**{st.session_state.org_name}** • Assessment Completed: {completed_date}")
    ...
    **Note:** This assessment identifies where decision confidence is well supported by current evidence 
    ...
    """.format(org_name=st.session_state.org_name, date=completed_date), unsafe_allow_html=True)
    ...
    # 1) Executive Brief (HTML)
    with col1:
        if st.button("📄 Build Executive Brief (HTML)", use_container_width=True):
            with st.spinner("Building your executive brief..."):
                # Prefer the Network Map for the artifact
                fig = create_network_signal_map(analysis)
                map_png_b64 = fig_to_png_base64(fig)

                html = build_executive_brief_html(
                    org_name=st.session_state.org_name,
                    assessment_date=str(completed_date),
                    analysis=analysis,
                    map_png_b64=map_png_b64
                )
                st.session_state._executive_brief_html = html
            st.success("Executive brief ready to download.")

        html_ready = st.session_state.get("_executive_brief_html")
        if html_ready:
            safe_org = "".join(c for c in st.session_state.org_name if c.isalnum() or c in (" ", "-", "_")).strip().replace(" ", "_")
            safe_date = str(completed_date).replace(" ", "_")
            st.download_button(
                label="⬇️ Download Executive Brief",
                data=html_ready,
                file_name=f"SIA_Executive_Brief_{safe_org}_{safe_date}.html",
                mime="text/html",
                use_container_width=True
            )

    # 2) Export raw data (JSON)
    with col2:
        payload = {
            "organization": st.session_state.org_name,
            "assessment_date_entered": str(st.session_state.assessment_date),
            "assessment_completed_date": str(completed_date),
            "responses": st.session_state.get("responses", {}),
            "analysis": analysis,
        }
        st.download_button(
            label="⬇️ Download Assessment Data (JSON)",
            data=json.dumps(payload, indent=2),
            file_name="SIA_Assessment_Data.json",
    ...
    # 3) Start over
    with col3:
        if st.button("↩️ Start New Assessment", use_container_width=True):
            st.session_state.page = "metadata"
            st.session_state.current_lifeline = 0
            st.session_state.responses = {}
            st.session_state.assessment_completed_date = None
